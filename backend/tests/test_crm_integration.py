...
class TestSalesforceIntegration:
    ...
    async def test_create_salesforce_contact_success(self, mock_session_class, crm_service):
        ...
        # Mock session
        mock_session = Mock()
        mock_session.post = AsyncMock(side_effect=[mock_auth_response, mock_contact_response])
    ...
    @pytest.mark.asyncio
    @patch('aiohttp.ClientSession')
    async def test_create_salesforce_contact_auth_failure(self, mock_session_class, crm_service):
...
class TestHubSpotIntegration:
    ...
    @pytest.mark.asyncio
    @patch('aiohttp.ClientSession')
    async def test_create_hubspot_contact_success(self, mock_session_class, crm_service):
        """Test successful HubSpot contact creation"""
        crm_service.hs_api_key = "test_api_key"
        
        # Mock search response (contact not found)
        ...
        mock_session = Mock()
        mock_session.post = AsyncMock(side_effect=[mock_search_response, mock_create_response])
        mock_session.__aenter__ = AsyncMock(return_value=mock_session)
        mock_session.__aexit__ = AsyncMock(return_value=None)
        mock_session_class.return_value = mock_session
    ...
    @pytest.mark.asyncio
    @patch('aiohttp.ClientSession')
    async def test_create_hubspot_contact_exists(self, mock_session_class, crm_service):
        """Test HubSpot contact creation when contact already exists"""
        crm_service.hs_api_key = "test_api_key"
        
        # Mock search response (contact found)
        mock_search_response = Mock()
        mock_search_response.status = 200
        mock_search_response.json = AsyncMock(return_value={
            "results": [
                {"id": "existing_contact_123", "properties": {"email": "john@example.com"}}
            ]
        })
        ...
        assert result["success"] is True
        assert result["contact_id"] == "existing_contact_123"
...
class TestCRMDataValidation:
    ...
    @pytest.mark.asyncio
    @patch('aiohttp.ClientSession')
    async def test_salesforce_contact_with_company(self, mock_session_class, crm_service):
        """Test Salesforce contact creation with company"""
        crm_service.sf_username = "test@example.com"
        crm_service.sf_password = "password123"
        crm_service.sf_security_token = "token123"
        ...
        mock_contact_response = Mock()
        mock_contact_response.status = 201
        mock_contact_response.json = AsyncMock(return_value={"id": "contact_123"})
    ...
    @pytest.mark.asyncio
    @patch('aiohttp.ClientSession')
    async def test_salesforce_contact_minimal_data(self, mock_session_class, crm_service):
